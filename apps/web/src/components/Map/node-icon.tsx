 ...
 * - `lastSync` null → 'unknown' (aucun CD n'a observé ce composant).
 * - sync < 1h → 'healthy'.
 * - sync < 24h → 'warning'.
 * - sync >= 24h → 'stale'.
 *
 * `now` est paramétrable pour les tests.
 */
...
export function buildNodeDivIcon(
  ...
  const healthBadge =
    health !== 'unknown'
      ? `<span aria-label="${HEALTH_LABEL[health]}" title="${HEALTH_LABEL[health]}" style="position:absolute;top:-2px;right:-2px;width:8px;height:8px;background:${HEALTH_COLOR[health]};border-radius:50%;border:1px solid #fff;"></span>`
      : '';
  ...
  const html = `<div data-kind="${kind}" data-default="${isDefaultPosition ? 'true' : 'false'}" data-selected="${selected ? 'true' : 'false'}" data-health="${health}" aria-label="${ariaLabel}" style="position:relative;width:24px;height:24px;background:${cfg.bgColor};border:2px solid #fff;border-radius:50%;display:flex;align-items:center;justify-content:center;box-sizing:border-box;${haloStyle}">${iconSvg}${healthBadge}${defaultPosBadge}</div>`;
